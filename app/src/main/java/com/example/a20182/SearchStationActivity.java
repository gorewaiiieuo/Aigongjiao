...
import com.example.a20182.searchStationUtils.BusStationInfo;
import com.example.a20182.searchStationUtils.stationAdapter;

import java.util.ArrayList;
import java.util.List;

import static com.example.a20182.FragmentHome.adcode;
import static com.example.a20182.FragmentHome.citycode;
import static com.example.a20182.FragmentHome.delBrac;
...
public class SearchStationActivity extends AppCompatActivity implements BusStationSearch.OnBusStationSearchListener {
        ...
        if(i==1000){
            ...
            for(BusStationItem item:list)
            {
                if(item!=null&&!item.getBusStationName().contains("地铁站")) {
                    ...
                    if(flag||item.getAdCode().equals(adcode)) {
                        ...
                        for (BusLineItem blitem : list2) {
                            strlist.add(delBrac(blitem.getBusLineName()).replace("区间", ""));
                        }
    ...
    private void setQuery(String name){
        BusStationQuery busStationQuery = new BusStationQuery(name, citycode);
